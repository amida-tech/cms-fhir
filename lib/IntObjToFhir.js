...
mapper["claim summary"] = function(memo, value, index, list) {
    _.forEach(memo.factory.claims(value, memo.patient, list[index]["claim number"], list[index]["source"]),
        function(val, key) {
            memo.bundle.entry.push(val);
        }
    );
...
Factory.prototype.claims = function(cms, patient, claimNumner, source) {
    var self = this;
    var result = [];
    ...
    var paymentReconciliationMapper = {
        ...
        "prescriber identifer": function(value) {
            if (!_.include(result, practitioner1)) {
                practitioner1.resource["id"] = (self.serial++).toString(),
                medicationDispense.resource["dispenser"] = {"refernece" :practitioner1 }; 
