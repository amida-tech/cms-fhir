...
mapper["claim summary"] = function(memo, value, index, list) {
    _.forEach(memo.factory.claims(value, memo.patient),
        function(val, key) {
            memo.bundle.entry.push(val);
        }
    );
...
mapper["claim"] = function(memo, value, index, list) {
    _.forEach(memo.factory.claims(value, memo.patient, list[index]["claim number"]),
        function(val, key) {
            memo.bundle.entry.push(val);
        }
    );
...
Factory.prototype.claims = function(cms, patient, claimNumner) {
    var self = this;
    var result = [];
    ...
    if (claim.resource.diagnosis.length > 0) {
        claim.resource.identifier.value = claimNumner;
        result.push(claim);
    }
