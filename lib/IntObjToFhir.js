...
function normaliseDateTime(value) {
    ...
    if(  denorm && !denorm[4]) { //Denormalized date only '31/12/2001'
        ...
        return denorm[3] + '-' + denorm[1] + '-' + denorm[2];
    }
    
    if(denorm) { //Denormalized date and time '31/12/2001 3:30 PM'
        //Switch to yyyy-mm-ddZHH:mm
        return denorm[3] + '-' + denorm[1] + '-' + denorm[2] + 'Z' + ((denorm[6] === 'AM')?denorm[4]: ( (partNorm[4] === '12')? '12': (parseInt( partNorm[4]) + 12).toString()  ) ) + ':'  + denorm[5];
    }
    ...
    if(partNorm ) { // Normalized date and denorm time '2001-12-31 3:30 PM'
        return partNorm[1] + '-' + partNorm[2] + '-' + partNorm[3] + 'Z' + ((partNorm[6] === 'AM')?partNorm[4]: ( (partNorm[4] === '12')? '12': (parseInt( partNorm[4]) + 12).toString()  ) ) + ':' + partNorm[5];
    }
