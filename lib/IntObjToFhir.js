 ...
 */
function Factory(patientId, baseUrl) {
    this.serial = 0;
    this.baseUrl = baseUrl;
    this.patientId = patientId;
}

function IntObjToFhirStream(patientId, baseUrl) {
    Transform.call(this, {
        "objectMode": true
    }); // invoke Transform's constructor, expected result is object
    this.factory = new Factory(patientId, baseUrl);

    this.bundle = this.factory.bundle();

}

var customMapper = function (a, b) {
    if (_.isUndefined(b)) {
        return a;
    }
};
...
function normaliseDateTime(value) {
    ...
    if (partNorm) { // Normalized date and denorm time '2001-12-31 3:30 PM'
        return partNorm[1] + '-' + align(partNorm[2]) + '-' + align(partNorm[3]) + 'T' + ((partNorm[6] === 'AM') ? partNorm[4] : ((partNorm[4] === '12') ? '12' : (parseInt(partNorm[4]) + 12).toString())) + ':' + partNorm[5] + ':00';
    }
...
Factory.prototype.claims = function (cms, patient, claimNumber, source, bundle) {
    ...
    var paymentReconciliationMapper = {
        ...
        "drug code": function (value) {
            ...
            medication.resource.code = _.extend({
                "coding": [{
        ...
        "prescriber identifer": function (value) {
            ...
            if (!practitioner1.resource["identifier"]) {
                practitioner1.resource["identifier"] = [];
            }
            practitioner1.resource["identifier"][0] = {
                "value": value
            };
            return {};
        },
            ...
            if (value !== "") {
                ...
                var location = {
                    "resource": {
                        ...
                        "physicalType": { "coding":[ (tmp.length === 2) ? {
                            "code": tmp[0],
                            "display": tmp[1]
                        } : {
                            "display": value
    ...
    _.forOwn(cms, function (value, key, object) {
        if (claimMapper[key]) {
            claim.resource = _.merge(claim.resource, claimMapper[key](value), customMapper);
        }
    });

    if (claim.resource.diagnosis.length > 0) {
        claim.resource.identifier[0].value = claimNumber;
        result.push(claim);
    }
    ...
    var diagnosticOrder = {
        "resource": {
            "resourceType": "DiagnosticOrder",
                "id": "DiagnosticOrder/" + (this.serial++).toString(),
            "subject": {
    ...
    if (source && source !== '') {
        var provenance = {
            "resource": {
                ...
                "id": "Provenance/" + (this.serial++).toString(),
                "agent": []
            }
        };
        ...
        provenance.resource.agent.push({
            'actor': {
                'reference': org.resource.id
            }
        });
        provenance.resource.target = [{'reference':diagnosticOrder.resource.id}];
...
Factory.prototype.drugs = function (cms, patient, empty, source) {
    ...
    var medicationStatementMapper = {
        "drug name": function (value) {
            medication = {
                "resource": {
                    "resourceType": "Medication",
                    "id": "Medication/" + (self.serial++).toString(),
                    //"name": value,
                    "code": {
                        "text": value
                    }
    ...
    var medicationOrderMapper = {
        "supply": function (value) {
            return {
                "dosageInstruction":[ normalizeDispense(value)]
            };
        }
    ...
    var medicationOrder = {
        "resource": {
            ...
            "id": "MedicationOrder/" + (self.serial++).toString(),
            "patient": {
                "reference": patient.resource.id
            }
        }
    ...
    var medicationDispense = {
        "resource": {
            ...
            "authorizingPrescription": [{
                "reference": medicationOrder.resource.id
            }]
        }
    };
    ...
    _.forOwn(cms, function (value, key, object) {
        if (medicationOrderMapper[key]) {
            medicationOrder.resource = _.merge(medicationOrder.resource, medicationOrderMapper[key](value));
        }
    });

    if (medication) {
        medicationOrder.resource.medicationReference = {
            "reference": medication.resource.id
        };
        result.push(medicationOrder);
    }
    ...
    _.forOwn(cms, function (value, key, object) {
        ...
        if (key.indexOf("description") >= 0) {
            if (familyMemeberHistory.resource["condition"][typeX]["note"]) {
                familyMemeberHistory.resource["condition"][typeX]["note"].text += '\n' + value;
            } else {
                familyMemeberHistory.resource["condition"][typeX]["note"] = {'text':value};
            }
        }
...
/**
 * @function
 * Generates Observation and may generate Practitioner, DiagnosticReport and DiagnosticOrder
...
Factory.prototype.selfReportedLabsAndTests = function (cms, patient) {
    ...
    if (diagnosticOrdertBase.resource.hasOwnProperty("note") || diagnosticOrdertBase.resource.hasOwnProperty("orderer")) {
    ...
    var diagnosticReportMapper = {
        "administered by": function (value) {
            ...
            var providederId = "Practitioner/" + (self.serial++).toString();
            administeredByBase["resource"]["name"]={
                "text": value
            };
        ...
        "date taken": function (value) {
            return {
                "issued": normaliseDateTime(value)
            };
        },
        "results": function (value) {
            return {
    ...
    if (diagnosticReportBase.resource.hasOwnProperty("conclusion") || diagnosticReportBase.resource.performer.length > 0) {
        if (this.baseUrl) {
            diagnosticReportBase["request"] = {
                "method": "POST",
                "url": "DiagnosticReport"
            };
            diagnosticReportBase["base"] = this.baseUrl;
            if (administeredByBase.resource.name) {
                result.push(administeredByBase);
            }

            if (diagnosticOrdertBase) {
                diagnosticReportBase.resource["request"] = [{
...
Factory.prototype.selfReportedMedicalConditions = function (cms, patient) {
    var selfReportedMedicalConditionsMapper = {
        ...
        "medical condition end date": function (value) {
            return {
                "abatementDateTime": normaliseDateTime(value)
            };
        }
    };
...
Factory.prototype.emergencyContact = function (cms, patient) {
    ...
    _.forOwn(cms, function (value, key, object) {
        if (emergencyContactMapper[key]) {
            contact = _.merge(contact, emergencyContactMapper[key](value), customMapper);
        }
    });
...
Factory.prototype.demographic = function (cms, empty1, empty2, source) {
    ...
    var patient = {
        "resource": {
            ...
            "id": "Patient/" + ((this.patientId) ? this.patientId : (this.serial++)),
    ...
    _.forOwn(cms, function (value, key, object) {
        if (patientMapper[key]) {
            patient["resource"] = _.merge(patient["resource"], patientMapper[key](value), customMapper);
        }
    });
