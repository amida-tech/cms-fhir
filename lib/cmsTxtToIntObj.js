...
matchers[SECTION_START] = [{
    ...
    proc: function(cps, match) {
        // Meta. Shift state
        cps.push2top("meta", META_HEADER, {"section header" : "meta"});
    }
}, {
    pattern: /^Claim Summary$/,
    proc: function(cps, match) {
        cps.push2top(match[0].toLowerCase(), SECTION_HEADER, {"section header" : match[0].toLowerCase()});
    }
}, {
    pattern: /^Claim Lines for Claim Number:\s*(.*)$/,
    proc: function(cps, match) {
        cps.push2top("claim number " + match[1], SECTION_HEADER, {"section header": "claim", "claim number": match[1]});
    }
}, {
    pattern: /^([^-]+)$/,
    proc: function(cps, match) {
        cps.push2top(match[1].toLowerCase(), SECTION_HEADER, {"section header" : match[1].toLowerCase()});
    }
}, {
    pattern: /^(-+)$/,
    proc: function(cps, match) {
        cps.push2top("empty", SECTION_HEADER, {"section header" : "empty"});
